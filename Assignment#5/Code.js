...
fn fs( @builtin(position) pos : vec4f ) -> @location(0) vec4f {
  ...
  let pcolor = clamp(0.0,1.0,p);
  
   if(p == 1.0){
      color = vec3f(1.0,0.5,0.0);
  }else if(p == 2.0){
     color = vec3f(0.3 + 1.0/(p+0.01),0.0,0.0);
  }else if(p == 3.0){
       color = vec3f(0.5,0.0,1.0);
   }else if(p == 4.0) {
       color = vec3f(0.0,1.0,1.0);
   }
   
  let out = select( vec3(pcolor) , color, v == 1. );
...
fn cs(@builtin(global_invocation_id) cell:vec3u)  {
  ...
  if( pheromone != 0.0 ) {      //  normal type
    if(vant.flag == 0.){
       vant.dir += .25; // turn 90 degrees counter-clockwise
       pheremones[ pIndex ] = 0.0;  // set pheromone flag
    }else if(vant.flag == 1.){   // 
       vant.dir -= .25;
       pheremones[ pIndex ] /= 2.0;
       if(pheremones[ pIndex ] < 0.1){
           pheremones[ pIndex ] = 0.0;
       }
    }else if(vant.flag == 2.){
       vant.dir +=0.25;
       pheremones[ pIndex ] /= 2;
  ...
  if(vant.flag == 0.){
    ...
    }else if(vant.flag == 3.0) {
       vant.dir += 0.25;
       pheremones[ pIndex ] = 4.0;
    }
  }
...
for( let i = 0; i < NUM_AGENTS * NUM_PROPERTIES; i+= NUM_PROPERTIES ) {
  ...
  //set three types of ants
  vants[i+3] = j++
  if(j>=4){
    j = 0;
  }
  
   // this could be used to hold vant "type"
}
